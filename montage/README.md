...
segments:
  - src: clips/talk.mov
    cleanup:                             # вырезать паузы, «э-э», неудачные дубли
      ...
      keep: [[40.2, 41.0]]               # принудительно оставить (не съедать окончание слова)
      relisten: [[55, 69]]               # переслушать участок точнее, если пропущены слова
    broll:                               # перебивки: картинка меняется, голос идёт
      ...
      - {src: clips/view.mov, at: 50.0, duration: auto}     # до следующей перебивки, встык
```
...
`speed: 1.1` в корне проекта ускоряет весь ролик: видео, голос без изменения тона, субтитры и заголовок вместе.
