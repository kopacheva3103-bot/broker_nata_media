...
const SHEET_ORDER = ['OBJ', 'STR', 'ACT', 'LEAD', 'STAT', 'PF', 'REP', 'DICT', 'DASH', 'CFG', 'CTRL', 'HIST', 'ARCH'];

const TAB_COLORS = {
  OBJ: '#37474F', STR: '#37474F', ACT: '#2E7D32', LEAD: '#2E7D32', STAT: '#1565C0',
...
const ALERT = {
  ...
  LEAD_NO_NEXT: 'Лид без следующего контакта',
...
function cfgDefs_() {
  return [
    ...
    { key: 'NO_LEADS_DAYS', label: 'Нет новых лидов дольше, дней', value: 14 },
    ...
    { key: 'CONV_DROP', label: 'Падение конверсии контакт→лид, считается значимым от (доля)', value: 0.3, fmt: '0%' },
    { key: 'MIN_CONTACTS', label: 'Мин. контактов в каждом окне для оценки падения конверсии', value: 10 },
    { key: 'LOST_SHARE', label: 'Много отказов: доля потерянных лидов от (доля)', value: 0.5, fmt: '0%' },
    { key: 'MIN_LEADS', label: 'Мин. лидов для оценки доли отказов', value: 3 },
...
const KPI_SOURCES = [
  ...
  { title: 'Заинтересованные', act: 'interested' },
  { title: 'Лиды', lead: true },
...
/** Коды отметок воронки у лида → поле-чекбокс в 04_ЛИДЫ_И_КОНВЕРСИИ. */
const LEAD_MARK_FIELDS = {
  QUAL: 'f_qual', PRES: 'f_pres', SHOW: 'f_show', NEG: 'f_neg', OFFER: 'f_offer', BOOK: 'f_book', DEAL: 'f_deal',
};
...
function dictDefs_() {
  return [
    ...
    {
      key: 'lead_sources', cols: ['Источник лида'], values: [
        ['ЦИАН'], ['Авито'], ['Яндекс Недвижимость'], ['CRM-база'], ['Брокер'], ['Партнёр'], ['Рекомендация'], ['Сайт'],
        ['Соцсети'], ['Реклама'], ['Outbound'], ['Мероприятие / клуб'], ['Входящий звонок'], ['Другое'],
      ],
    },
    {
    ...
    {
      key: 'lead_status', cols: ['Статус лида', 'Класс', 'Отметить этапы'], values: [
        ['Новый', 'OPEN', ''], ['Контакт', 'OPEN', ''], ['Квалифицирован', 'OPEN', 'QUAL'],
        ['Презентация', 'OPEN', 'QUAL,PRES'], ['Показ', 'OPEN', 'QUAL,SHOW'], ['Повторный контакт', 'OPEN', 'QUAL'],
        ['Переговоры', 'OPEN', 'QUAL,NEG'], ['Предложение', 'OPEN', 'QUAL,OFFER'], ['Бронь', 'OPEN', 'QUAL,BOOK'],
        ['Сделка', 'WON', 'QUAL,DEAL'], ['Потерян', 'LOST', ''], ['Отложен', 'PAUSED', ''],
      ],
    },
    {
    ...
    {
      key: 'loss_reasons', cols: ['Причина потери'], values: [
        ['Выбрал другой объект'], ['Пропал / нет связи'], ['Не прошёл по бюджету'], ['Отложил покупку'],
        ['Не согласовали цену'], ['Ушёл к другому агенту'], ['Нецелевой'], ['Другое'],
      ],
    },
    { key: 'client_types', cols: ['Тип клиента'], values: [['Покупатель'], ['Инвестор'], ['Брокер покупателя'], ['Партнёр'], ['Представитель / семейный офис'], ['Другое']] },
